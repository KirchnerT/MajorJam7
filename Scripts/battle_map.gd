...
var ally_units: Array[String] # Change "String" to the base unit class
var enemy_units: Array[String] # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
func _ready() -> void:
	## TEST CODE
	var test_container = UnitContainerInfo.new(load(temp_soldier_resource), 1, 0, 0, 0, Vector2(1,1))
